# Used for benchmarking we can make changes here and see how they affect perf.
defmodule Elexer.EmitterNoHalt do
  @space " "
  @speech_mark "\""
  @negative_symbol "-"
  @open_paren "("
  ...
  defp emitt_number_event(number, handler, state, rest_of_source_code) do
    case handler.handle_event(@number_arg_event, number, state) do
      {:ok, new_state} -> rest_of_source_code |> parse_args(handler, new_state)
    end
  end

  defp emitt_number_events(number, handler, state, rest_of_source_code) do
    case handler.handle_event(@number_arg_event, number, state) do
      {:ok, state} -> emit_close_fn_event(handler, rest_of_source_code, state)
    end
  end

  defp emit_close_fn_event(handler, rest_of_source_code, state) do
    case handler.handle_event(@close_fn_event, state) do
      {:ok, new_state} -> parse_args(rest_of_source_code, handler, new_state)
    end
  end
